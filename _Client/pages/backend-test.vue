<template>
  <div style="padding: 20px; max-width: 800px;">
    <h1><IconRocket style="width: 1.2rem; height: 1.2rem; vertical-align: -0.15rem;" /> 실제 백엔드 API 테스트</h1>
    ...
    <div style="background: #f5f5f5; padding: 15px; border-radius: 5px; margin-bottom: 20px; max-height: 400px; overflow-y: auto;">
      <h3><IconChart style="width: 1rem; height: 1rem; vertical-align: -0.15rem;" /> API 응답 로그</h3>
      ...
      <ul v-else style="list-style: none; padding: 0;">
        <li v-for="(result, idx) in testResults" :key="idx" style="margin: 10px 0; padding: 10px; background: white; border-left: 4px solid #2196F3; border-radius: 3px;">
          ...
          <details style="margin-top: 5px;">
            <summary style="cursor: pointer; color: #2196F3;"><IconFileText style="width: 0.95rem; height: 0.95rem; vertical-align: -0.15rem;" /> 응답 데이터</summary>
    ...
    <div style="background: #fff3e0; padding: 15px; border-radius: 5px;">
      <h3><IconTarget style="width: 1rem; height: 1rem; vertical-align: -0.15rem;" /> 브라우저 콘솔 명령어</h3>
...
const testBackendApis = async () => {
  ...
  try {
    console.log('%c백엔드 API 테스트 시작', 'color: #FF5722; font-weight: bold; font-size: 14px;');
    ...
    console.log('%cAPI 테스트 완료!', 'color: #4CAF50; font-weight: bold; font-size: 14px;');
    console.log('%c요청 로그는 프론트 logs/api-front.log, 백엔드 logs/api-back.log 에 저장됩니다.', 'font-style: italic; color: #FF9800;');
