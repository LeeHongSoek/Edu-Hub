...
INSERT INTO `questions` (`question_id`, `creator_no`, `group_id`, `question_type_id`, `title`, `question`, `content`, `answer`, `explanation`, `hint`, `difficulty`, `is_public`, `time_limit`, `is_deleted`) VALUES
(1, 1, 3, 'M', 'OSI 7계층 - 전송 계층 (4지선다)', 'OSI 7계층 중 종단 간(End-to-End) 신뢰성 있는 정보 전송을 담당하는 계층은?', NULL, '', '전송 계층(Transport Layer)은 TCP와 UDP 프로토콜이 대표적이며 오류 복구와 흐름 제어 등을 담당합니다.', 'TCP/UDP가 속한 계층', 3, TRUE, 60, 'N'),
(2, 1, 3, 'M', '디자인 패턴 - 생성 (5지선다)', '다음 중 소프트웨어 객체 지향 디자인 패턴의 생성(Creational) 패턴이 아닌 것은?', NULL, '', 'Observer 패턴은 행위(Behavioral) 패턴에 속합니다.', '상태 변화를 관찰하는 패턴은 생성과 무관합니다.', 4, TRUE, 120, 'N'),
(3, 3, 9, 'M', '영어 단어 - Abandon (4지선다)', '다음 중 영어 단어 ''Abandon''의 뜻으로 가장 알맞은 것은?', NULL, '', '어원은 프랑스어 abandonner (넘겨주다)에서 유래했으며, give up과 유사한 의미를 가집니다.', 'give up과 비슷한 뜻', 1, TRUE, 30, 'N'),
(4, 2, 6, 'S', '파이썬 - 리스트 기초 (주관식)', '파이썬에서 비어있는 리스트를 생성하는 코드를 두 가지 작성하시오.', NULL, '[] 또는 list()', '대괄호를 사용하거나 내장 함수 list()를 사용하여 빈 리스트를 만들 수 있습니다.', '대괄호 사용', 2, TRUE, 180, 'N'),
(5, 1, 11, 'M', '물리 - 뉴턴 제2법칙 (긴 지문형)', '수식과 내용에 따르면, 질량 $m$이 2배로 증가할 때 가속도 $a$는 어떻게 변하는가?', '다음 지문을 읽고 물음에 답하시오.\n\n> "물체의 가속도는 작용한 힘에 비례하고 질량에 반비례한다."\n\n이를 뉴턴의 제2법칙이라 부르며, 수식으로는 다음과 같이 나타낼 수 있다.\n\n$$ F = m \\cdot a $$\n\n여기서 힘 $F$가 $\\underline{\\text{일정}}$하게 유지될 때, 다음 물음에 답하라.', '', '수식 $a = \\frac{F}{m}$ 에 대입하여 관계를 파악하세요.', '마크다운과 수식을 렌더링하는 테스트입니다.', 4, TRUE, 300, 'N'),
(6, 1, 13, 'M', '고등 수학 - 정적분 (라텍스 수식형)', '다음 주어지는 미적분 문제의 정적분 수행 결과값으로 알맞은 것은?', '함수 $f(x) = x \\sin(x)$ 일 때, 정적분 $\\int_{0}^{\\frac{\\pi}{2}} f(x) dx$ 의 값을 구하시오.\n\n(참고: 필요하다면 부분적분법 $\\int u v'' dx = uv - \\int u'' v dx$ 을 활용하시오.)', '', '부분적분 $u=x, v''=\\sin(x)$로 치환하면 $u''=1, v=-\\cos(x)$이므로, $[-x\\cos(x)]_{0}^{\\frac{\\pi}{2}} + \\int_{0}^{\\frac{\\pi}{2}} \\cos(x)dx$ = $0 + [\\sin(x)]_{0}^{\\frac{\\pi}{2}}$ = $1$이 됩니다.', '부분적분법을 활용', 5, TRUE, 600, 'N');
...
-- 15. 문제 평점 및 리뷰
INSERT INTO `question_reviews` (`question_id`, `user_no`, `content`, `rating`, `created_at`) VALUES
